...
#let conf(
  ...
  if abstract != none {
    heading(outlined: false, numbering: none, text(normal-size, weight: "regular", [摘要]))
    align(center)[
      #block(
        width: 90%,
        [
  ...
  // Author Note
  if not anonymous {
    heading(outlined: false, numbering: none, text(normal-size, weight: "bold", [作者说明]))
    ...
    // Contact Information
    if type(authors-parsed.corresponding) == dictionary and "email" in authors-parsed.corresponding and authors-parsed.corresponding.email != "" [
      [通信作者：#authors-parsed.corresponding.name。]
      if "postal" in authors-parsed.corresponding and authors-parsed.corresponding.postal != "" [#authors-parsed.corresponding.postal。]
      [邮箱：#link("mailto:" + authors-parsed.corresponding.email, authors-parsed.corresponding.email)]
    ]
  }
