...
| Cycle logs | active | `cycles/CYCLE-20260502-006.md` |
...
| Branch shortlist decision surface | draft ready for user decision | `planning/BRANCH_SHORTLIST_DECISION_SURFACE.md` |
...
Choose finalist branches from `planning/BRANCH_SHORTLIST_DECISION_SURFACE.md`:
...
```text
Return to A / B / C. Cycle 005 added primary-source anchors for the previously weakest areas (visual priors, depth priors, active perception, event VO). Cycle 006 reorganized the workspace into topical subdirectories (paradigm/, planning/, sources/, units/, handoff/, logs/, archive/). Templates for proxy case cards and finalist mechanism specs are ready under `templates/`. Another round of mining (D again) should only run if a specific missing comparator blocks a chosen branch.
```
