...
| `STYLE_CONTRACT.md` | Cycle 036 P0-B-2 + cycle 037 §6 sync log + cycle 038 §2 +19 rows + cycle 039 §2 +2 rows + cycle 040 §2 +5 rows deliverable: bilingual style contract. §2 vocabulary substitution table 48 rows total (Dream3R → 候选架构 X / SPEC-* → 体系结构设计文档 v0.X / DEC-* → 项目关键决策点 N / CYCLE-* → 研发周期 N / CR-1..CR-6 → 信号校验规则族 (1-6) / W1..W22 → 实现里程碑 1-22 / F-001/F-002 → 内部工作规则 / 算力部署约束 (远端服务器执行) / agent / skill / workflow / 本地项目 / KYKT → omit + rephrase / ablate_recurrence.py / ABL-memory-N / ABL-v02-N → 中文化 / AnchorBank K=256 / NSA three-branch / StateToken / Mamba hybrid / pointmap L2 = 20.47 / 4DGS asset → 中文化 + 上下文标注 / C1-C6 六模块中文化 + Pillar A/D + Delta N + repair actions 0-5 + conflict_score / theta_conflict / capability_match spread / cost_adjusted_match / epsilon_tie / fail_fast / evidence label / forward-reference null protocol / permanence_link / signal names retain English + C4' / Innovation Point IP1/IP2/IP3 / hard_fail / soft_fail / oracle-bus / monotone upgrade gate / fixture regime R1-R5 / 集成证据). §3 sync rule internal-is-master + 周期性 external snapshot + standalone external 编辑限制 + per-sync grep verification. §4 候选架构 X 命名引入规则. §5 candidate-not-final 句式对比表 (9 禁用 → 允许 句式 对照). §6 append-only sync log (cycle 036 → 037 → 038 → 039 → 040 entries) |
| `DRAFT_INTERNAL_V1.md` | Cycle 036 P0-B-3 + cycle 037 §2 + cycle 038 §4 + cycle 039 §3 + §6 + cycle 040 §5 + §7 + §8 deliverable: Dream-vocabulary 内部稿. §1 ~1800 字 (Track A 主线决策起源 + Dream3R v0.3 当前状态 + 综述四轴反哺 + 三个核心研究问题 + 候选 vs 最终边界 + 工件引用); §2 ~4200 字 (7 sub-sections); §3 ~1800 字 (5 sub-sections: Q1/Q2/Q3 + 4-finalist 独立性 + 候选 vs 最终 研究地位); §4 ~4000 字 (8 sub-sections: 整体设计 + 帧预算 + C1-C6 六模块 + 与现有 3R 系统的结构差异); §5 ~2800 字 (7 sub-sections: 三层证据阶梯 / ABL-v02-1..10 / ABL-memory-0..11 / Critic 标定 / 长序列真实评测 / 评测数据集 / 主要评测指标); §6 ~1300 字 (3 sub-sections: 预期交付物 + 三个 IP 声明 + 与现有工作的实证差异); §7 ~2200 字 (6 sub-sections: SPEC v0.1/v0.2/v0.3 三系列 + W1-W18 + KITTI 集成证据 + 综述发布 + 综述反哺 + cycle 历史); §8 ~1500 字 (3 sub-sections: 短期 M1-M2 / 中期 M3-M5 / 长期 M6-M8); §9 TBD placeholder. 累计正文 ~17800 字. master per STYLE_CONTRACT §3 |
| `DRAFT_EXTERNAL_V1.md` | Cycle 036 P0-B-4 + cycle 037 §2 + cycle 038 §4 + cycle 039 §3 + §6 + cycle 040 §5 + §7 + §8 deliverable: academic-Chinese 外部稿 (vocab-clean per G3a + G3b; 候选架构 X 命名 + 六模块中文化命名 + 评测协议术语中文化 per §4 STYLE_CONTRACT). §1 ~1500 字; §2 ~3700 字 (mirrors §2 internal structure); §3 ~1500 字; §4 ~3000 字 (六模块中文化); §5 ~2000 字 (7 sub-sections mirroring DRAFT_INTERNAL §5 with hard_fail/soft_fail → 硬失败/软失败 + oracle-bus → 理想信号总线 + fixture regime R1-R5 → 校准 fixture 制度 等 5 vocab substitutions); §6 ~1000 字; §7 ~1500 字 (6 sub-sections mirroring DRAFT_INTERNAL §7; § 7.3 KITTI 集成证据 vs 训练后质量 boundary 显式保留); §8 ~1000 字 (3 sub-sections; M-numbering 显式声明 candidate timeline 不是 committed schedule); §9 TBD placeholder. 累计正文 ~14000 字. periodic snapshot of internal per STYLE_CONTRACT §3 |
