...
| `STYLE_CONTRACT.md` | Cycle 036 P0-B-2 + cycle 037-041 sync log + §2 incremental rows deliverable: bilingual style contract. §2 vocabulary substitution table 50 rows total (Dream3R → 候选架构 X / SPEC-* → 体系结构设计文档 v0.X / DEC-* → 项目关键决策点 N / CYCLE-* → 研发周期 N / CR-1..CR-6 → 信号校验规则族 (1-6) / W1..W22 → 实现里程碑 1-22 / F-001/F-002 → 内部工作规则 / 算力部署约束 (远端服务器执行) / agent / skill / workflow / 本地项目 / KYKT → omit + rephrase / ablate_recurrence.py / ABL-memory-N / ABL-v02-N → 中文化 / AnchorBank K=256 / NSA three-branch / StateToken / Mamba hybrid / pointmap L2 = 20.47 / 4DGS asset → 中文化 + 上下文标注 / C1-C6 六模块中文化 + Pillar A/D + Delta N + repair actions 0-5 + conflict_score / theta_conflict / capability_match spread / cost_adjusted_match / epsilon_tie / fail_fast / evidence label / forward-reference null protocol / permanence_link / signal names retain English + C4' / Innovation Point IP1/IP2/IP3 / hard_fail / soft_fail / oracle-bus / monotone upgrade gate / fixture regime R1-R5 / 集成证据). §3 sync rule internal-is-master + 周期性 external snapshot + standalone external 编辑限制 + per-sync grep verification. §4 候选架构 X 命名引入规则. §5 candidate-not-final 句式对比表 (9 禁用 → 允许 句式 对照). §6 append-only sync log (cycle 036 → 037 → 038 → 039 → 040 entries) |
| `DRAFT_INTERNAL_V1.md` | Cycle 036-041 deliverable: Dream-vocabulary 内部稿. §1-§9 全部完整 (~19300 字): §1 ~1800 字 + §2 ~4200 字 + §3 ~1800 字 + §4 ~4000 字 + §5 ~2800 字 + §6 ~1300 字 + §7 ~2200 字 + §8 ~1500 字 + §9 ~1500 字 (6 sub-sections: 架构层/跨模块/实证执行/工程时序/开题报告 process/风险应对策略). master per STYLE_CONTRACT §3 |
| `DRAFT_EXTERNAL_V1.md` | Cycle 036-041 deliverable: academic-Chinese 外部稿 (vocab-clean per G3a + G3b; 候选架构 X 命名 + 六模块中文化命名). §1-§9 全部完整 (~15000 字): §1 ~1500 字 + §2 ~3700 字 + §3 ~1500 字 + §4 ~3000 字 + §5 ~2000 字 + §6 ~1000 字 + §7 ~1500 字 + §8 ~1000 字 + §9 ~1000 字 (5 sub-sections: 架构层/跨模块/实证执行/工程时序/风险应对策略; §9.5 process 风险 omitted per vocab-clean rule). periodic snapshot of internal per STYLE_CONTRACT §3 |
...
| File | Cycle | Role |
|---|---|---|
| `CYCLE-20260517-002.md` | 041 | Dream3R 开题报告 § 9 风险分析与应对 dual-draft + 通稿审查 + STYLE_CONTRACT final sync (DEC-20260517-002 authorized; DRAFT_INTERNAL_V1 § 9 ~1500 字 6 sub-sections + DRAFT_EXTERNAL_V1 § 9 ~1000 字 5 sub-sections vocab-clean; STYLE_CONTRACT 48→50 rows; G3a 0 + G3b 0 + G4 1 corrective edit then 0; 通稿审查 clean; § 1-§ 9 全部章节完整 ~19300 内 + ~15000 外 字; no spec / code / server action) |
