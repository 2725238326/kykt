...
Last updated: 2026-05-16 (cycle 036 closed — DEC-20260516-001 authorized dual-scope packaging + proposal kickoff; 3 new files in `3R-mix/deliverables/` package the Track B Chinese survey for advisor / school internal review; 4 new files in new subdirectory `planning/proposal_dream3r/` launch the Chinese Dream3R 开题报告 dual-draft scaffold (§ 1 alignment proof complete; § 2-§ 9 placeholders for cycle 037+); WORK_RISK_REGISTER v1.1 → v1.2 additive +3 proposal-cycle risk rows; G2 + G3a + G3b + G4 vocab firewall + over-claim greps all 0 hits; Track A Dream3R v0.3 server-verified through cycle 034 unchanged; Track B 3R-mix 2026-05-15 prose naturalization deliverable remains canonical PDF for actual submission)
...
| `DRAFT_INTERNAL_V1.md` | Cycle 036 P0-B-3 deliverable: Dream-vocabulary 内部稿. §1 ~1800 字 covers §1.1 Track A 主线决策起源 (DEC-20260506-001 + 5 in-force constraints) + §1.2 Dream3R v0.3 当前状态 (架构定型 SPEC-004 v0.2 6 模块 / 比较图谱 SPEC-007 v0.2 5 tier + Axis 9-11 / 消融计划 SPEC-005 v0.2 + SPEC-008 v0.3 + SPEC-009 / 代码实装 W1-W18 部署服务器 path / KITTI 真实集成证据 L2=20.47 / ablation 现状 / 现有 gated 项) + §1.3 Track B 综述四轴反哺 (cycle 035 4 deliverables) + §1.4 三个核心研究问题 Q1 验证机制路径 (Critic) + Q2 长序列内存路径 (Memory) + Q3 多专家组合路径 (Composer) + §1.5 候选 vs 最终边界 + §1.6 Dream 项目工件引用. §2-§9 TBD placeholders with 子节 suggestions. master per STYLE_CONTRACT §3 |
| `DRAFT_EXTERNAL_V1.md` | Cycle 036 P0-B-4 deliverable: academic-Chinese 外部稿 (vocab-clean per G3a + G3b; 候选架构 X 命名 per §4 STYLE_CONTRACT). §1 ~1500 字 covers §1.1 前馈式三维重建 (3R) 研究方向 + §1.2 六类典型几何失败模式 (弱纹理 / 镜面玻璃 / 快速运动 / 长基线 / 长序列累积尺度漂移 / OOD) + §1.3 三组未充分解决问题 (验证 vs 适应 + 长序列内存机制统一 + 多专家组合实证) + §1.4 本研究目标 (代号 候选架构 X 引入 + 4 设计目标) + §1.5 研究地位 (candidate-not-final + 不押注单一方案) + §1.6 研究价值与学术意义 (方法学 + 实证 + 工程 3 方面贡献). §2-§9 TBD placeholders. periodic snapshot of internal per STYLE_CONTRACT §3 |
...
| What is the next user decision? | `WORKFLOW_STATUS.md` -> Recommended Next User Decision; current default after cycle 036 close is one of: A. launch cycle 037 § 2 国内外研究现状 dual-draft drafting (recommended) / B. user executes actual Track B survey submission action + fills SUBMISSION_RECORD slots / C. revise OUTLINE_V1 chapter structure first / D. pause + revise cycle 036 deliverables / E. return to architecture-first mainline non-proposal work / F. launch cycle 035 §Next Direction A-C (calibration / long-seq ablation / v0.4 spec delta) |
...
| What is the latest research result? | newest file under `cycles/`; current latest is `cycles/CYCLE-20260516-001.md` (cycle 036 done; advisor submission packaging + Dream3R 开题报告 dual-draft kickoff with § 1 alignment proof + WORK_RISK_REGISTER v1.2 +3 proposal-cycle rows + sync chain; preceded by cycle 035 survey-driven markdown deliverables + 4 cross-spec risk register additions; full Track A cycle 034 KITTI real-data smoke + Mamba/Gaussian + Track B 3R-mix kickoff history preserved) |
