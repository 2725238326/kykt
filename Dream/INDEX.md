...
Last updated: 2026-05-17 (cycle 039 closed — DEC-20260516-004 authorized § 3 候选研究问题 + § 6 预期成果与创新点 dual-draft drafting; DRAFT_INTERNAL_V1 + DRAFT_EXTERNAL_V1 § 3 ~1800 内 + ~1500 外 字 across 5 sub-sections (Q1/Q2/Q3 + 4-finalist 独立性 + 候选 vs 最终 研究地位) + § 6 ~1300 内 + ~1000 外 字 across 3 sub-sections (预期交付物 + 三个 IP 声明 + 与现有工作的实证差异); § 1 + § 2 + § 3 + § 4 + § 6 累计 ~11300 内 + ~9500 外 字 ≈ 54% of OUTLINE_V1 §2 表 总字数估算; § 5 / § 7 / § 8 / § 9 placeholders for cycles 040-041; STYLE_CONTRACT §2 vocab table 41→43 rows (+2: C4' TTT 路径模块代号 / Innovation Point IP1/IP2/IP3); §6 sync log appended; G3a + G3b 0 hits on first pass; G4 7 hits per side from negation-context candidate-not-final 句式 examples, applied 7 corrective edits per side per cycle 036 precedent, re-grep all 0 hits; cycle 038 + 037 + 036 + 035 + 034 + Track A Dream3R v0.3 server-verified through cycle 034 + Track B 3R-mix 2026-05-15 deliverable + SHA256 pre-filled all unchanged)
...
New subdirectory created cycle 036 (per DEC-20260516-001). Hosts the Chinese Dream3R 开题报告 dual-draft scaffold: Dream-vocabulary 内部稿 + academic-Chinese 外部稿 (uses 代号 "候选架构 X" / "本研究架构"; no raw "Dream3R" in main text). § 1 of both drafts complete as style-and-vocabulary alignment proof; § 2-§ 9 placeholders for cycle 037+. The actual Track B survey submission and this 开题报告 are sibling artifacts that share `references.bib` material but do not cite each other.
...
| `STYLE_CONTRACT.md` | Cycle 036 P0-B-2 deliverable: bilingual style contract. §2 vocabulary substitution table 22 rows (Dream3R → 候选架构 X / SPEC-* → 体系结构设计文档 v0.X / DEC-* → 项目关键决策点 N / CYCLE-* → 研发周期 N / CR-1..CR-6 → 信号校验规则族 (1-6) / W1..W22 → 实现里程碑 1-22 / F-001/F-002 → 内部工作规则 / 算力部署约束 (远端服务器执行) / agent / skill / workflow / 本地项目 / KYKT → omit + rephrase / ablate_recurrence.py / ABL-memory-N / ABL-v02-N → 中文化 / AnchorBank K=256 / NSA three-branch / StateToken / Mamba hybrid / pointmap L2 = 20.47 / 4DGS asset → 中文化 + 上下文标注). §3 sync rule internal-is-master + 周期性 external snapshot + standalone external 编辑限制 + per-sync grep verification. §4 候选架构 X 命名引入规则. §5 candidate-not-final 句式对比表 (9 禁用 → 允许 句式 对照). §6 append-only sync log (cycle 036 entry: 13 vocab substitutions seeded + § 1 grep clean) |
| `DRAFT_INTERNAL_V1.md` | Cycle 036 P0-B-3 + cycle 037 §2 + cycle 038 §4 + cycle 039 §3 + §6 deliverable: Dream-vocabulary 内部稿. §1 ~1800 字 (Track A 主线决策起源 + Dream3R v0.3 当前状态 + 综述四轴反哺 + 三个核心研究问题 + 候选 vs 最终边界 + 工件引用); §2 ~4200 字 (7 sub-sections: 基础谱系 / 多视角规模化 / 视频动态 4D / 长序列内存四类 B1-B4 / 测试时三类 C1-C3 / 输出资产三类 D1-D3 / 综述四轴覆盖矩阵 + Dream3R v0.3 落点); §3 ~1800 字 (5 sub-sections: 3.1 Q1 验证机制 + 3.2 Q2 长序列内存 + 3.3 Q3 多专家组合 + 3.4 4-finalist 模块独立性 + 3.5 候选 vs 最终 研究地位); §4 ~4000 字 (8 sub-sections: 整体设计 + 帧预算 + C1-C6 六模块 + 与现有 3R 系统的结构差异; 含 7 专家池 booktabs 表 + CR-1..CR-6 引用 + Delta 1/2/3/5/6 引用 + W17/W18 锚点); §6 ~1300 字 (3 sub-sections: 6.1 预期交付物 + 6.2 三个 IP 声明 (Q1↔IP1 / Q2↔IP3 / Q3↔IP2) + 6.3 与现有工作的实证差异); §5 / §7 / §8 / §9 TBD placeholders. 累计正文 ~11300 字. master per STYLE_CONTRACT §3 |
| `DRAFT_EXTERNAL_V1.md` | Cycle 036 P0-B-4 + cycle 037 §2 + cycle 038 §4 + cycle 039 §3 + §6 deliverable: academic-Chinese 外部稿 (vocab-clean per G3a + G3b; 候选架构 X 命名 + 六模块中文化命名 per §4 STYLE_CONTRACT). §1 ~1500 字; §2 ~3700 字 (mirrors §2 internal structure with vocab-clean substitutions); §3 ~1500 字 (5 sub-sections mirroring DRAFT_INTERNAL §3 with 候选架构 X 命名 + 信号校验规则族 referencing CR-1..CR-6); §4 ~3000 字 (8 sub-sections mirroring DRAFT_INTERNAL §4 with 感知 / 记忆 / 永久性 / 校验 / 编排 / 总线 模块名 + paraphrased Delta 引用 omitted + 信号校验规则族六条 replacing CR-1..CR-6); §6 ~1000 字 (3 sub-sections mirroring DRAFT_INTERNAL §6 with vocab-clean 三个 IP 命名 + 候选不等于最终 句式); §5 / §7 / §8 / §9 TBD placeholders. 累计正文 ~9500 字. periodic snapshot of internal per STYLE_CONTRACT §3 |
...
| What is the next user decision? | `WORKFLOW_STATUS.md` -> Recommended Next User Decision; current default after cycle 039 close is one of: A. launch cycle 040 § 5 实验设计 + § 7 已完成工作 + § 8 时间安排 together (recommended; three implementation-anchored chapters ~11000 字 total) / B. revise § 3 + § 6 based on self-review or advisor feedback / C. launch cycle 035 §Next Direction A-C (calibration / long-seq ablation / v0.4 spec delta) / D. pause + reassess after § 3 + § 6 quality review / E. user executes Track B survey submission (SHA256 pre-filled) / F. return to architecture-first mainline non-proposal work |
...
| What is the latest research result? | newest file under `cycles/`; current latest is `cycles/CYCLE-20260516-004.md` (cycle 039 done; Dream3R 开题报告 § 3 候选研究问题 + § 6 预期成果与创新点 dual-draft 起草 ~1800 内 + ~1500 外 字 (§3) + ~1300 内 + ~1000 外 字 (§6); § 1 + § 2 + § 3 + § 4 + § 6 累计 ~11300 内 + ~9500 外 字 ≈ 54% target; STYLE_CONTRACT §2 vocab table 41→43 rows; §6 sync log appended; G3a + G3b 0 hits on first pass; G4 7 hits per side from candidate-not-final negation-context examples, applied 7 corrective edits per side per cycle 036 precedent, re-grep all 0 hits; preceded by cycle 038 § 4 dual-draft + cycle 037 § 2 dual-draft + cycle 036 advisor packaging + dual-draft kickoff + cycle 035 survey-driven markdown deliverables) |
