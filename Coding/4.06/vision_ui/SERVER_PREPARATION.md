...
- local runner exists at `runners/monst3r_runner.py`.
- remote execution is now wired to call the official `demo.py` through `conda run -n monst3r`.
- the client can save MonST3R-specific parameters and download the actual remote output/log tree.
...
- Required checkpoints are now present:
...
- A residual interrupted `download_ckpt.sh` / `wget models.zip` process was stopped on 2026-04-10.
...
- Steps 1 to 4 are done enough for the first demo attempt.
- Step 5 is the active blocker: run one real video or frame-sequence example and inspect the artifacts.
...
### Current MonST3R output contract

The remote runner currently standardizes the official demo output by copying useful
artifacts from the demo sequence folder into the per-job `output/` folder:

- `output/*.glb` for the reconstructed 3D scene when produced.
- `output/pred_traj.txt` and `output/pred_intrinsics.txt` when produced.
- saved RGB / depth / confidence / mask images from the official demo.
- `.npy` intermediate geometry arrays when produced.
- `output/scene_meta.json` generated by the KYKT runner.
- `logs/runner.log`
- `status.json`
...
- outputs: at least one `.glb` or saved image group + `scene_meta.json` + logs
