...
- 2026-04-11: A tiny remote MonST3R smoke run was executed against the uploaded video path. The model loaded, read frames, finished pair inference, and started exporting GLB / trajectory, but the official demo then failed because `third_party/RAFT/models/raft-things.pth` is still missing on the server. This is the current concrete blocker for the first end-to-end MonST3R success.
